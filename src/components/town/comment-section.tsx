...
export function CommentSection({ townId, initialComments }: { townId: string, initialComments: TownComment[] }) {
  ...
  const CommentNode = ({ comment, depth = 0 }: { comment: TownComment, depth?: number }) => (
    <div className={`flex gap-4 ${depth > 0 ? "ml-8 mt-4 border-l border-wiki-border pl-4" : "mt-6"}`}>
      ...
      <div className="flex-1 space-y-2">
        <div className="flex justify-between items-baseline">
          <span className="font-semibold text-wiki-text text-sm">{comment.author.name || "Anonymous"}</span>
        ...
        <div className="flex gap-4 items-center">
          {session?.user && depth < 2 && ( // Limit depth to 2 to prevent extreme nesting
            <button 
        ...
        {replyingTo === comment.id && (
          <div className="mt-3 space-y-2 animate-in fade-in slide-in-from-top-2">
            ...
            <div className="flex justify-end gap-2">
              ...
              <Button size="sm" onClick={() => handleReply(comment.id)} disabled={loading || !replyContent.trim()}>
  ...
  return (
    <div className="space-y-8 mt-16 pt-8 border-t border-wiki-border">
      ...
      <div className="space-y-2">
        {comments.length === 0 ? (
          <p className="text-wiki-muted text-center py-8">No comments yet. Be the first to share!</p>
        ) : (
