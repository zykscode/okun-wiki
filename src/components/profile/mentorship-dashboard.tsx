...
export function MentorshipDashboard({
  ...
  const handleAction = async (action: (id: string) => Promise<void> | void, id: string) => {
    setLoadingId(id);
    try {
      await action(id);
    } catch (e) {
                ...
                <Card key={m.id}>
                  <CardContent className="pt-6 flex justify-between items-center">
                    <div>
                      <Badge
                        variant="outline"
                        className="mb-2 text-yellow-600 bg-yellow-50 border-yellow-200"
                      >
                        PENDING
                      </Badge>
                      <p className="text-sm font-medium">
                        Looking for a {m.menteeId === userId ? "Mentor" : "Mentee"}
                      </p>
                    ...
                    <Button
                      ...
                      onClick={() => handleAction(cancelMentorship, m.id)}
                      disabled={loadingId === m.id}
                    >
        ...
        <div>
          ...
          {availableRequests.length === 0 ? (
            <p className="text-wiki-muted text-sm">
              No opportunities currently available in your communities.
            </p>
          ) : (
            <div className="space-y-4">
              {availableRequests.map((m) => {
                ...
                return (
                  <Card key={m.id} className="bg-blue-50/50 border-blue-100">
                    <CardContent className="pt-6">
                      <div className="mb-2">
                        <span className="font-semibold text-wiki-text">
                          {requesterName || "A member"}
                        </span>
                        <span className="text-sm text-wiki-secondary">
                          {" "}
                          is looking for a {lookingFor} in {m.community.name}.
                        </span>
                      </div>
                      <Button
                        size="sm"
