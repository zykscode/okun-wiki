      ...
      <div className="flex flex-col lg:flex-row gap-8">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="mb-6">
            ...
            {town.tagline && <p className="text-lg text-wiki-muted mt-2">{town.tagline}</p>}
          </div>
                          ...
                          <div className="flex gap-2 overflow-x-auto pb-2 mt-3 no-scrollbar">
                            {festival.images.map(
                              (img: { id: string; url: string; caption: string | null }) => (
                                <div
                                  key={img.id}
                                  className="w-40 h-28 shrink-0 rounded-lg overflow-hidden border border-wiki-border"
                                >
                                  <img
                                    src={img.url}
                    ...
                    {town.people.map((person) => (
                      ...
                      >
                        <div className="flex items-center gap-2 mb-2">
                          {person.title && <Badge variant="primary">{person.title}</Badge>}
                          ...
                          {town.needs.map((need) => (
                            ...
                            >
                              <div className="flex justify-between items-center mb-1">
                                ...
                                <span className="text-xs text-wiki-muted font-medium">
                                  {need.category}
                                </span>
                              </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                          {town.families.map((family) => (
                            ...
                            >
                              ...
                              <p className="text-xs text-wiki-muted mt-1 mb-2 line-clamp-3">
                                {family.history}
                              </p>
                              {family.headOfFamily && (
                                <p className="text-xs font-medium">
                                  Head:{" "}
                                  <span className="text-wiki-text">{family.headOfFamily}</span>
                                </p>
                              )}
                            </div>
                          ))}
                        ...
                        <div className="flex flex-wrap gap-2">
                          {town.neighbors.map((n) => (
                            <Link key={n.id} href={`/towns/${n.neighbor.slug}`}>
                              <Badge
                                variant="outline"
                                className="px-3 py-1.5 hover:bg-gray-50 cursor-pointer flex items-center gap-1 transition-colors"
                              >
                                {n.neighbor.name}
                                {n.distanceKm && (
                                  <span className="text-wiki-muted ml-1 text-xs">
          ...
          {/* Comments Section */}
          <div className="mt-12">
            <CommentSection townId={town.id} initialComments={town.comments} />
          </div>
        </div>
      </div>
    </div>
