...
export default async function UserProfilePage({ params }: Props) {
  ...
  let myRequests: MentorshipItem[] = [];
  let activeConnections: MentorshipItem[] = [];
  let availableRequests: MentorshipItem[] = [];
          ...
          <div className="flex items-center gap-6">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-1 flex-wrap">
                <Badge
                  variant={
                    user.role === "ADMIN" ? "primary" : user.role === "EDITOR" ? "earth" : "default"
                  }
                >
                  {user.role}
                </Badge>
                {user.isApprovedWriter && (
                  <Badge
                    variant="outline"
                    className="border-gold-500 text-gold-600 bg-gold-50 dark:bg-gold-900/20"
                  >
                    Approved Writer
                  </Badge>
                )}
                  ...
                  <div className="flex gap-2 flex-wrap">
                    {user.mentorshipTracks.map((track, i) => (
                      <Badge
                        key={i}
                        variant="default"
                        className="bg-forest-100 text-forest-700 dark:bg-forest-900/30 dark:text-forest-300"
                      >
                        {track.name}
                      </Badge>
                    ))}
              ...
              <div className="space-y-3">
                {user.createdTowns.map((town) => (
                  <Link
                    ...
                    className="block no-underline hover:bg-wiki-hover rounded-lg p-2 -mx-2 transition-colors"
                  >
